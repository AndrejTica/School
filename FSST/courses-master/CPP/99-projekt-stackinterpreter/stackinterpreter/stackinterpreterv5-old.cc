...
int CStackinterpreter::run(){
	       ...
	       while (m_pc < m_nol) {
			...
			if (str[0]=='/' && str[1]=='/' )		//Kommentar
				...
				// ermittle begin des schleifen blockes
				while_begin= m_pc;
				//int nextline=m_pc;
				//while (m_code[nextline]!="endwhile")
				//	nextline++;
				//nextline zeigt nun auf zeile mit endwhile
			}
        		else if (str=="endwhile" && while_again==false){
				;
			}
        		else if (str=="endwhile" && while_again==true){
				m_pc=while_begin;
			}			
			else if (str=="lt") {
				 if (m_stackengine.lt())
